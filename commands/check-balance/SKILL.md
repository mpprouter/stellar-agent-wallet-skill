...
npx tsx commands/check-balance/swap-xlm-to-usdc.ts 10
...
# Check any pubkey
npx tsx commands/check-balance/run.ts GABCD...

# JSON output
npx tsx commands/check-balance/run.ts --json
```
...
npx tsx commands/check-balance/swap-xlm-to-usdc.ts --usdc 5 --slippage 0.005

# Skip confirmation on mainnet (large-amount gate still fires)
npx tsx commands/check-balance/swap-xlm-to-usdc.ts --xlm 10 --yes
```
